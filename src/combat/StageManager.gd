...
func _open_portals() -> void:
	...
	if current_stage in [4, 8, 9, 10]:
		...
		p.setup(PortalScript.PortalType.STANDARD, current_world)
	...
	p_combat.setup(PortalScript.PortalType.COMBAT, current_world)
	...
	p_sustain.setup(PortalScript.PortalType.SUSTAIN, current_world)
